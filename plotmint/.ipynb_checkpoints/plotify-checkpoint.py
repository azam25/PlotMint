...
def vis_seasonalPlot(df, date_col, value_col, fig_width = 12, fig_height = 6, title = '', title_fontsize = 16,
    ...
    for i, y in enumerate(years):
        plt.plot(date_col + '_month', value_col, data=df.loc[df[date_col + '_year']==y, :], color=mycolors[i], label=y)
        plt.text(df.loc[df[date_col + '_year']==y, :].shape[0]-.9, df.loc[df[date_col + '_year']==y,value_col][-1:].values[0], y, fontsize=12, color=mycolors[i])
...
def vis_divergence(df, diverging_col, y_axix_col, fig_width = 14, fig_height = 10, text_size = 10, yticks_font_size = 12, fig_title = '', title_text_size = 20,
    ...
    for x, y, tex in zip(df[diverging_col + '_z'], df.index, df[diverging_col + '_z']):
        t = plt.text(x, y, round(tex, 2), horizontalalignment='right' if x < 0 else 'left', 
                     verticalalignment='center', fontdict={'color':'red' if x < 0 else 'green', 'size':text_size})
...
def vis_piechart(df, groupby_col, fig_width=8, fig_height=8, title='', title_fontsize =14,annotation_textsize = 12,
    ...
    df.plot(kind='pie', subplots=True, figsize=(fig_width, fig_height), fontsize = annotation_textsize, autopct='%1.1f%%')
...
def vis_density_curve(df, category_col, value_col, fig_width = 12, fig_height = 6, title = '', title_fontsize = 16,
    ...
    colors = [plt.cm.Spectral(i/float(len(cat_list)-1)) for i in range(len(cat_list))]
...
def vis_stackedAreaPlot(df,x_col,y_col_list, fig_width=12,fig_height=7, title='', title_fontsize=14, legend_fontsize =10, xtick_fontsize=10,
    ...
    # List Colors 
    colors = [plt.cm.Spectral(i/float(len(df.columns)-1)) for i in range(len(df.columns))]   
...
def vis_unstackedAreaPlot(df, x_col, y_col_list, fig_width=12,fig_height=7, title='', title_fontsize=14, legend_fontsize =10, xtick_fontsize=10,
    ...
    # Decide Colors 
    colors = [plt.cm.Spectral(i/float(len(y_col_list)-1)) for i in range(len(y_col_list))]      
